...
def create_data(package: str, wind_units: str):
    ...
    if package == "iris":
        result = iris.cube.Cube(
            data, units=wind_units,
            dim_coords_and_dims=(
                (iris.coords.DimCoord(lats, "latitude", units="degrees_north"), 0),
                (iris.coords.DimCoord(lons, "longitude", units="degrees_east", circular=True), 1)
            )
        )
    elif package == "xarray":
        result = xr.DataArray(
...
@pytest.mark.parametrize(
    "package", [pytest.param("iris", marks=pytest.mark.skipif("iris is None")),
